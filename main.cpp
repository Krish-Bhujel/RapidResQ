...
static  string severityName(IncidentSeverity s)
{
    switch (s)
    {
...
int main()
{
    ...
    const float MAP_WIDTH = 900.f;
    const float MAP_HEIGHT = 900.f;
    ...
    if (!loadAll())
    {
         cerr << "Fatal: could not load initial data." <<  endl;
        return 1;
    }
    ...
    {
         cerr << "Fatal: could not load scenario." <<  endl;
        return 1;
    }
    ...
    renderer.loadDecorationIcon("hospital1", "../assets/hospital_1.png", 70.f);
     ...
     vector<DecorationEntry> decorations;
    if (!DecorationLoader::load("../data/decorations.txt", decorations)) {
         cerr << "Warning: could not load decorations.txt." <<  endl;
    }
    ...
    // Fixed color per ambulance (cycled if more ambulances than colors), used for path overlays.
     vector<sf::Color> ambulanceColors = {
     ...
     unordered_map<int, sf::Color> colorByAmbulanceId;
    ...
    while (window.isOpen())
    {
            ...
            if (const auto *mousePressed = event->getIf<sf::Event::MouseButtonPressed>())
            {
                if (mousePressed->button == sf::Mouse::Button::Left)
                {
                    ...
                    if (clickPos.x < MAP_WIDTH)
                    {
                        ...
                        if (hitNode != -1)
                        {
                            IncidentSeverity sev = static_cast<IncidentSeverity>( rand() % 3);
                            ...
                            if (bestFrom != -1 && bestDist <= CLICK_THRESHOLD)
                            {
                                ...
                                if (currentlyBlocked)
                                {
                                    city.unblockEdge(bestFrom, bestTo);
                                     cout << "Road unblocked (manual click): " << bestFrom << " <-> " << bestTo <<  endl;
                                }
                                else
                                {
                                    city.blockEdge(bestFrom, bestTo);
                                     cout << "Road blocked (manual click): " << bestFrom << " <-> " << bestTo <<  endl;
                                }
                            }
                        }
                    }
        ...
        if (appState == AppState::Running && !sim.isFinished())
        {
            if (elapsedSinceLastTick >= TICK_INTERVAL_SECONDS)
            {
                ...
                {
                     cout << "[Tick " << log[i].tick << "] " << log[i].message <<  endl;
                }
        ...
        // ---- Path overlays (drawn before ambulance icons, on top of roads) ----
         vector<Simulation::AmbulanceRenderState> ambStates(ambulances.size());
         unordered_map<int,  vector<size_t>> parkedByNode;
        ...
        // ---- Incident beacons ----
        float pulsePhase =  fmod(pulseClock.getElapsedTime().asSeconds(), 1.5f) / 1.5f;
        ...
        for (size_t i = 0; i < ambulances.size(); ++i)
        {
            ...
            if (state.isMoving && uiFontLoaded)
            {
                ...
                etaText.setString("ETA: " +  to_string(state.ticksRemainingTotal));
        ...
        float infoY = 236.f;
        auto drawInfoLine = [&](const  string &text, sf::Color color = sf::Color(220, 220, 225))
        {
            if (!uiFontLoaded)
                return;
            sf::Text t(uiFont);
        ...
        drawInfoLine("Tick: " +  to_string(sim.currentTick()));
        ...
        drawInfoLine("  Idle: " +  to_string(idleCount));
        drawInfoLine("  En route: " +  to_string(enRouteCount));
        drawInfoLine("  On scene: " +  to_string(onSceneCount));
        drawInfoLine("  Returning: " +  to_string(returningCount));
        infoY += 10.f;

        drawInfoLine("Active Incidents:", sf::Color(255, 180, 120));
         vector<Incident> activeIncidents = sim.getActiveIncidents();
        ...
        drawInfoLine("Blocked Roads:", sf::Color(255, 140, 140));
         vector< pair<int, int>> blocked = city.getBlockedEdges();
        if (blocked.empty())
        {
            drawInfoLine("  None");
        }
        else
        {
            ...
            {
                drawInfoLine("  " +  to_string(b.first) + " <-> " +  to_string(b.second));
            }
        }
